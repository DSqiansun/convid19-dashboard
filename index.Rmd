...
```{r}
valueBox(format(sum(data$confirmed), big.mark=","))
...
valueBox(format(sum(data$recovered), big.mark=","))
```

### Deceased {.value-box}

```{r}
valueBox(format(sum(data$deceased), big.mark=","))
...
```{r}
valueBox(format(sum(data$active), big.mark=","))
```

### Death rate {.value-box}

```{r}
valueBox(paste0(as.character(format(round(sum(data$deceased)/sum(data$confirmed)*100, 2),nsmall=1, big.mark=",")), '%'))
```
